import{fetchPlaceholders as v,getMetadata as b}from"../../scripts/aem.min.js";import{branchURLStr as y,CFApiCall as L}from"../../scripts/scripts.min.js";import{dropDownStateCity as x,locateMeClick as S,onloadBranchLocator as C}from"./branchlocator-biz.min.js";import{setLocationObj as l}from"./branchlocator-init.min.js";export function branchLocator_dropdown(t){return`<div class='container dropdown-container'> 
            ...
            <div class='cards-container'>
        ...
        </div>`}export async function innerBranchFunc(t){let a="";const r=await v();return t.forEach(e=>{const n=e.State,i=e.City,d=e["Location Code"],s=e.Location;a+=`<div class='card-box'>
              ...
              <a href="${y(s,i,n,"loans",d)}" id='more-details-btn'>${r.moredetailtext}  </a> 
            </div>`}),a}export default async function $(t){const a=await v(),r=Array.from(t.children,s=>s.firstElementChild),[e]=r,n=e.textContent.trim(),d=(await L(n))?.data;sessionStorage.getItem("data")?l.getExcelData=JSON.parse(sessionStorage.getItem("data")):(l.getExcelData=x(d),sessionStorage.setItem("data",JSON.stringify(l.getExcelData))),t.innerHTML=branchLocator_dropdown(t),t.innerHTML+=branchLocator_Map(t,a),t.innerHTML+=branchLocator(),C(t),S(t),M(t)}function M(t){const{state:a,city:r}=l.geoInfo;if(!a)return;const e=(o,c=!1)=>{const m=o.toLowerCase().replace(/\s+/g,"-");return c?m.charAt(0).toUpperCase()+m.slice(1):m},n=e(a),i=e(a,!0),s='<span class="breadcrumb-separator"><svg width="6" height="10" viewBox="0 0 6 10" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 9.00195L4.29293 5.70902C4.68182 5.32013 4.68182 4.68377 4.29293 4.29488L1 1.00195" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path></svg></span>',h=[`<a href="${b("primary-language-path")}/branch-locator/${n}">${i}</a>`];if(r){const o=e(r),c=e(r,!0);h.push(`<a href="${b("primary-language-path")}/branch-locator/${n}/${o}">${c}</a>`)}const g=h.map(o=>`${s}${o}`).join("");t.closest("body").querySelector(".breadcrumb nav").insertAdjacentHTML("beforeend",g);let w=t.closest("body").querySelectorAll(".breadcrumb nav a");const u=[];w.forEach((o,c)=>{u.push({"@type":"ListItem",position:c+1,name:o.textContent,item:location.origin+o.getAttribute("href")})});let f={"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:u};const p=document.createElement("script");p.type="application/ld+json",p.innerHTML=JSON.stringify(f),document.head.append(p)}
